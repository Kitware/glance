...
var createWebworkerPromise = function createWebworkerPromise(name, existingWorker) {
  if (existingWorker) {
    ...
    return Promise.resolve({ webworkerPromise: _webworkerPromise, worker: existingWorker });
  }

  var webWorkerUrl = config.itkModulesPath + '/WebWorkers/' + name + '.worker.js';
  if (webWorkerUrl.startsWith('http')) {
    return axios.get(webWorkerUrl, { responseType: 'blob' }).then(function (response) {
      ...
      return { webworkerPromise: webworkerPromise, worker: worker };
    });
  }

  var worker = new window.Worker(webWorkerUrl);
  var webworkerPromise = new WebworkerPromise(worker);
  return Promise.resolve({ webworkerPromise: webworkerPromise, worker: worker });
};
